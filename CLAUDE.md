...
- **Running a suite**: the detail page's "Run" button POSTs to `test_suite_run`, which calls `_run_test_suite()` — this is the one place `testomatic-ui` imports the sibling `testomatic-runner` package (`-e ../testomatic-runner` in `requirements.txt`) as a library, in-process, not a subprocess: `testomatic.suite.load_suite()` parses the downloaded package, then `testomatic.runner.TestRunner(chassis, test_module, avrdude_path=..., ...).run(suite)` executes it against real hardware (`testomatic_io.Chassis`/`TestModule`, importable only on a real Raspberry Pi — an `ImportError` here is reported to the operator as "Test Runner hardware support is not available on this device" rather than crashing the page). The 4 `avrdude_path`/`esptool_path`/`openocd_path`/`stm32cubeprogrammer_path` keyword arguments come from this device's own `core.models.DeviceSettings` singleton (see Device Settings below) — testomatic-runner's `ExecutionContext` defaults each to the bare tool name on `$PATH` when blank/`None`. The report is rendered once, on the same page, and isn't stored anywhere yet.

## Device Settings

`core.models.DeviceSettings` (`get_solo()`, same singleton pattern as Register's `erp.AssemblyCostSettings`) holds this physical device's own configuration, as opposed to a Test Suite's configuration, which comes from Register and is identical regardless of which device runs it. Currently just the 4 firmware-upload tool executable paths (`avrdude_path`/`esptool_path`/`openocd_path`/`stm32cubeprogrammer_path`) — blank means "use the bare tool name on this device's `$PATH`". Edited via `/settings/` (`core.views.device_settings_edit`, staff-only, linked from the sidebar as "Device Settings"), a single plain form (`core.forms.DeviceSettingsForm`) since it's a singleton — same "one edit page, no list page" pattern as Register's `assembly_cost_settings_edit`. `test_suites.views._run_test_suite()` is the only consumer, reading it once per run and passing the 4 paths through to `TestRunner(...)`.
