...
Edited via `/settings/` (`core.views.device_settings_edit`, staff-only, linked from the sidebar as "Device Settings"), a single plain form (`core.forms.DeviceSettingsForm`) covering both cards since it's one singleton with one save button — `device_settings_edit.html` splits the same form's fields across two `<div class="card">` blocks (via the `core/_settings_field.html` include, one per field) purely for presentation, not two separate forms.
