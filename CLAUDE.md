...
`core` has `OperatorProfile` (see Operator Login below) and `DeviceSettings` (see Device Settings below). `test_suites` has `Design` and `TestSuite` — see Test Suites below. Login is Django's standard session-based `django.contrib.auth.urls`, but authentication itself is handled by `core.auth_backends.RegisterAuthBackend` (see below), not a plain local password check.
...
- **`test_suites/test_suite_package.py`** (issue #3) — parses a fetched `TestSuite.package_file`'s ZIP (`test-suite-definition.json`) into `StepDisplay`/`CheckDisplay` objects for the read-only `/test-suites/<pk>/` detail page (`test_suite_detail` view, 404s if not yet fetched — only downloaded versions are clickable from the list page). `STEP_TYPE_LABELS`/`STEP_TYPE_COLORS`/`_config_summary()` are an **exact port** of Register's `testing.models.TestStep` (`STEP_TYPE_CHOICES`/`STEP_TYPE_COLORS`/`get_config_summary()`) — keep the two in sync by hand if Register adds/changes a step type; an unrecognised `step_type` falls back to a generic grey badge rather than crashing. `StepDisplay`/`CheckDisplay` expose the same `get_color()`/`get_step_type_display()`/`get_config_summary()` method names Register's own template calls, so `test_suites/templates/test_suites/detail.html` is a close structural copy of Register's read-only `testing/test_suite_version_detail.html` — same bordered/colored card-per-step layout, deliberately with no drag handle, delete button, or `onclick` navigation (the whole point of the page, per issue #3, is that nothing on it is editable or clickable). Register's `UPLOAD_FIRMWARE_*` step types (4 tools, since issue #121) are ported here the same way — `UPLOAD_FIRMWARE_AVRDUDE`/`_ESPTOOL`/`_OPENOCD`/`_STM32CUBEPROGRAMMER`, sharing one colour, same as Register.
- **Running a suite**: the detail page's "Run" button POSTs to `test_suite_run`, which calls `_run_test_suite()` — this is the one place `testomatic-ui` imports the sibling `testomatic-runner` package (`-e ../testomatic-runner` in `requirements.txt`) as a library, in-process, not a subprocess: `testomatic.suite.load_suite()` parses the downloaded package, then `testomatic.runner.TestRunner(chassis, test_module, avrdude_path=..., ...).run(suite)` executes it against real hardware (`testomatic_io.Chassis`/`TestModule`, importable only on a real Raspberry Pi — an `ImportError` here is reported to the operator as "Test Runner hardware support is not available on this device" rather than crashing the page). The 3 `avrdude_path`/`openocd_path`/`stm32cubeprogrammer_path` keyword arguments come from this device's own `core.models.DeviceSettings` singleton (see Device Settings below) — testomatic-runner's `ExecutionContext` defaults each to the bare tool name on `$PATH` when blank/`None`. There is no `esptool_path` here: testomatic-runner's `pi` extra installs `esptool` from PyPI directly, so its `esptool.py` console script is already on `$PATH` with no device-side override needed. `_run_test_suite()` returns a `RunResult` dataclass (not a bare tuple) carrying the formatted text/pass-fail/error *and* the raw `testomatic.runner.RunReport` plus `manual_checks` — the structured data the storage/docket steps below need, kept alive past this function rather than being discarded.
- **Serial number entry** (issue #8, ahead of the Test Docket work it's for): `test_suite_run` requires a `serial_number` POST field, re-rendering the detail page with `serial_number_error` instead of running if it's blank. A barcode scanner feeds the run page's Serial Number input, and does double duty — a plain barcode scan yields the bare serial number, but the QR code (which also serves as a customer-facing link) yields a full URL instead. `test_suites/serial_number.py:extract_serial_number(raw_input, url_stem)` strips `DeviceSettings.device_details_url_stem` (see Device Settings below) back off in the URL case, returning the bare serial number either way.
- **Result storage** (issue #6, local slice only) — `test_suites.models.TestRun` is created by `_save_test_run()` whenever `_run_test_suite()` actually got a `RunReport` back (pass, fail, or aborted all count; a run that couldn't even start — e.g. missing hardware support — creates nothing, same as before). `TestRun.report` is a `JSONField` holding `dataclasses.asdict()` of every `StepOutcome` plus `aborted` and the suite's `manual_checks` — the "detailed, structured, grouped by Test Step" data issue #6 asks for, ready for a future batch-upload transport to Register to consume without re-running anything. That network upload itself doesn't exist yet — this is local storage only.
- **Test Docket formatting/printing** (issue #8) — `test_suites/docket.py` builds the docket. `build_docket_lines()` produces the plain-text content (header, automatic checks from `RunReport.outcomes`, a manual-checks checklist, a footer with the test-suite version and the device-details URL), styled after the legacy prototype's own output (see `3990-MCM-*.txt`, a real production docket from the system this replaces). `render_docket_image()` then draws those lines plus a QR code (`qrcode`/Pillow) onto a **single bitmap** — not plain text printed separately from a QR image — so the whole thing prints as one seamless receipt via `print_docket_image()`'s `lp -d <printer_name> <path>` call. Canvas width (576px) and font size assume 80mm thermal paper at 203dpi and a monospace TTF at a common Raspberry Pi OS font path, falling back to Pillow's low-quality built-in bitmap font if that path is missing — **unverified against real thermal-printer hardware**, expect to need tuning once it has been tested on one. **Firmware versions (register#124):** there's no dedicated firmware-version field anywhere in testomatic-ui/testomatic-runner/Register — the resolved decision (per register#124's discussion) is that `build_docket_lines()` collects every `UPLOAD_FIRMWARE_*` step's own `name` and lists them under the `H/W version:` line as a `Firmware:` section, using `testomatic.steps.firmware.FIRMWARE_STEP_TYPES` (imported directly from testomatic-runner, not re-duplicated) to pick those steps out of `RunReport.outcomes` as a special case. Every matching step is listed, including one whose image is later overwritten by a subsequent firmware step in the same run (e.g. a test/bootloader image flashed early, replaced by the production image at the end) — nothing collapses multiple firmware steps down to "the" final version. This is independent of `include_on_docket`: a firmware step's name always appears here (suffixed ` (FAILED)` if the upload failed) regardless of that flag, since it reports what's physically on the board, not a pass/fail line — unlike the ordinary per-step line in "Automatic checks", which a passing step can opt out of via `include_on_docket`. The practical implication for Test Suite authors: a firmware step's **name** is what shows up on the docket, so it should read like a version (e.g. "Firmware v8.1.1"), not a generic label like "Upload Firmware" — see register's `docs/user-guide/test-suites.md`. `_print_test_run_docket()` always saves `TestRun.docket_text`/`docket_image` regardless of whether the actual `lp` call succeeds (recording `docket_print_error` on failure), and only attempts to print at all when `DeviceSettings.printer_name` is set. `docket_image` is the source of truth for reprints — the "Reprint Docket" button (`test_run_reprint` view) resends that exact saved file rather than re-rendering, so a historical docket's appearance can't drift if `docket.py`'s rendering changes later.

## Device Settings

`core.models.DeviceSettings` (`get_solo()`, same singleton pattern as Register's `erp.AssemblyCostSettings`) holds this physical device's own configuration, as opposed to a Test Suite's configuration, which comes from Register and is identical regardless of which device runs it. Three unrelated groups of fields, shown as separate cards on the same `/settings/` page:

- The 3 firmware-upload tool executable paths for the tools with no PyPI distribution (`avrdude_path`/`openocd_path`/`stm32cubeprogrammer_path`) — blank means "use the bare tool name on this device's `$PATH`". `esptool` is deliberately not one of these fields: unlike avrdude/OpenOCD/STM32CubeProgrammer, it ships on PyPI, so testomatic-runner's `pi` extra installs it (and its `esptool.py` console script) straight onto `$PATH` — see testomatic-runner's own `CLAUDE.md`/`pyproject.toml`. `test_suites.views._run_test_suite()` is the only consumer, reading it once per run and passing the 3 paths through to `TestRunner(...)`.
- `printer_name` (issue #7) — the CUPS print queue this device prints a test docket to (e.g. `Printer_POS-80`), for a `lp -d <printer_name> ...` call once the docket-printing feature itself exists (issue #6). Not consumed by anything yet — issue #7 deliberately split this field off from #6 as a self-contained first step, ahead of the printing logic.
- `device_details_url_stem` (issue #8) — the base URL this device prepends to a DUT's serial number to build the customer-facing device details link/QR code on the Test Docket (e.g. `https://d.superlab.au/` + `12345`), and the same stem `test_suites.serial_number.extract_serial_number()` strips back off a QR-code scan on the run page (see Serial number entry above). Deliberately just one fallback value, not per-customer/per-design yet — some customers want a vanity redirect URL instead, which is real but explicitly deferred; a future per-customer/per-design override would still need this field as its fallback.

Edited via `/settings/` (`core.views.device_settings_edit`, staff-only, linked from the sidebar as "Tester Settings" with Register's own `cil-task` icon — chosen to match Register's "Testers" sidebar item, since this page is this device's own tester-side settings; the model/view/URL name stayed `DeviceSettings`/`device_settings_edit`, only the user-facing menu label, page heading, and icon changed), a single plain form (`core.forms.DeviceSettingsForm`) covering all three cards since it's one singleton with one save button — `device_settings_edit.html` splits the same form's fields across three `<div class="card">` blocks (via the `core/_settings_field.html` include, one per field) purely for presentation, not separate forms.

Deliberately holds nothing for the serial port / debug-probe identification fields (`UPLOAD_FIRMWARE_AVRDUDE`'s `port`, `UPLOAD_FIRMWARE_OPENOCD`'s `adapter_serial`, etc.) — those stay entirely suite-side, read straight out of the Test Suite Package's own `config`, exactly as Register defines them. Whether some of those should instead (or additionally) become device-side config here is an open question, tracked as Register issue #122 — deliberately deferred until the system has been exercised end-to-end using the current suite-side-only approach.
...
Still early: a themed shell, the Register API client, the Test Suites list/fetch/detail feature (issues #1, #2, #3), Design thumbnails on that list (Register issue #117), Register-backed operator login with an offline fallback (issue #4), running a fetched Test Suite Package against real hardware via testomatic-runner (see "Running a suite" above), per-device firmware-upload tool path/printer/device-details-URL configuration (Device Settings above, issues #7/#8), serial-number capture on the run page, local result storage (issue #6, storage only — no network upload to Register yet), and Test Docket formatting/printing including a QR code (issue #8) exist. All of it is unverified against real hardware/tools — in particular the Test Docket's image rendering has not been tried against a real thermal receipt printer — see testomatic-runner's own `TEST_RUNNER_PLAN.md` for what's confirmed vs. not on the runner side. The GitHub repo is [github.com/SuperHouse/testomatic-ui](https://github.com/SuperHouse/testomatic-ui).
